...
export default function ProyeccionMetas() {
  ...
  const [miembrosEquipo, setMiembrosEquipo] = useState({}) // { asesorId: true/false }
  ...
  useEffect(() => {
    ...
    supabase
      .from('equipo_comercial_miembros')
      .select('asesor_id, activo')
      .then(({ data }) => {
        const mapa = {}
        ;(data || []).forEach((m) => {
          mapa[m.asesor_id] = m.activo
        })
        setMiembrosEquipo(mapa)
      })
  }, [])

  const toggleMiembroEquipo = async (asesorIdToggle) => {
    const nuevoValor = !miembrosEquipo[asesorIdToggle]
    setMiembrosEquipo((prev) => ({ ...prev, [asesorIdToggle]: nuevoValor }))
    setGuardandoEquipo(true)

    const { error } = await supabase
      .from('equipo_comercial_miembros')
      .upsert(
        { asesor_id: asesorIdToggle, activo: nuevoValor, updated_at: new Date().toISOString() },
  ...
  return (
    <div>
      ...
      {/* Filtro de miembros del equipo, solo si scope = grupal */}
      {scope === 'grupal' && (
        <div className="bg-white rounded-xl shadow-sm p-4 mb-4">
          <h3 className="text-sm font-semibold text-gray-700 mb-1">Miembros del equipo comercial</h3>
          <p className="text-xs text-gray-500 mb-2">
            Marca qué asesores cuentan para la Meta grupal. Esta selección aplica igual para los 4 periodos.
          </p>
          <div className="flex flex-wrap gap-2">
            {asesores.map((a) => {
              const activo = !!miembrosEquipo[a.id]
              return (
                <label
                  key={a.id}
                  className={`text-xs px-3 py-1.5 rounded-full border cursor-pointer ${
                    activo ? 'bg-brand-600 text-white border-brand-600' : 'bg-white text-gray-600 border-gray-300'
                  }`}
                >
                  <input
                    type="checkbox"
                    ...
                    onChange={() => toggleMiembroEquipo(a.id)}
                  />
                  {a.nombre || 'Sin nombre'}
                </label>
              )
